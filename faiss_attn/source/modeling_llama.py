...
class LlamaFlashAttention2(LlamaAttention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # exit()
        # print("here")
        # print(kwargs.get('block_list', None))
        ...
        # exit()
        # exit()
        # block_list = [[11, 15], [16, 19], [8, 26], [6, 16], [7, 12], [17, 22], [7, 10], [19, 15], [6, 9], [21, 30], [24, 29], [11, 2], [6, 30], [18, 30], [21, 1], [15, 14], [10, 18], [17, 18], [22, 22], [17, 0], [14, 7], [20, 30], [19, 10], [14, 24], [20, 0], [7, 4], [21, 4], [21, 16], [20, 29], [22, 19], [24, 30], [11, 14], [20, 10], [13, 23], [22, 8], [29, 19], [20, 28], [23, 31], [18, 10], [11, 30], [23, 20], [8, 22], [24, 3], [14, 29], [16, 1], [14, 18], [8, 31], [12, 16], [21, 28], [20, 1]]

        # for h in block_list:
        #     if self.layer_idx==h[0]:
        #         query_states[:,h[1], :, :] = query_states[:,h[1], :, :] * 1.5
        #         # attn_weights[:, h[1], :, :] = 0 
        ...
        if 'block_list' in kwargs:
            ...
            for h in kwargs['block_list']:
                if self.layer_idx==h[0]:
                    # print(query_states[:,h[1], :, :].mean())
                    # print(query_states[:,h[1], :, :].std())
                    # print(query_states[:,h[1], :, :].max())
                    # print(query_states[:,h[1], :, :].min())
                    # exit()
                    #
                    # query_states[:,h[1], :, :] = query_states[:,h[1], :, :] *
                    # 只有在 attn_weights 不为 None 时才进行赋值
                    # 如果 output_attentions=False，attn_weights 会是 None
        ...
        if input_dtype == torch.float32:
            ...
            if hasattr(self.config, "_pre_quantization_dtype"):                target_dtype = self.config._pre_quantization_dtype
            else:
