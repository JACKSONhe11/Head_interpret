...
# TruthfulQA is optional for activation collection; load lazily to avoid hard dependency.
try:
    ...
    INCORRECT_COL = None
    format_prompt = None
    format_prompt_with_answer_strings = None
    split_multi_answer = None
    format_best = None
    find_start = None
    preset_map = None
    COMPARE_PRIMER = None
    find_subsequence = None
    set_columns = None
    MC_calcs = None
    format_frame = None
    data_to_dict = None
# OpenAI is optional - only needed for GPT-3 model evaluation
try:
    import openai
except ImportError:
    openai = None  # Will be checked before use if needed
    ...
    with torch.no_grad():
        ...
        if is_custom_llama:
            ...
            try:
                ...
                if hasattr(first_layer, 'forward'):
                    ...
                    has_attn_mode = 'attn_mode' in sig.parameters
                    # 检查 self_attn 的类型
                    if has_attn_mode and hasattr(first_layer, 'self_attn'):
                        from source.modeling_llama import LlamaFlashAttention2
                        # 只有当 self_attn 是 LlamaFlashAttention2 时才使用 attn_mode="torch"
                        use_attn_mode_torch = isinstance(first_layer.self_attn, LlamaFlashAttention2)
                    else:
                        is_custom_llama = False
            except:
                is_custom_llama = False
                use_attn_mode_torch = False
        ...
        try:
            with TraceDict(model, HEADS + MLPS) as ret:
                ...
                # 如果是 LlamaAttention，不能使用 attn_mode 参数（会导致返回值数量不匹配）
                if is_custom_llama and use_attn_mode_torch:
                    output = model(prompt, output_hidden_states=True, attn_mode="torch")
                else:
                    # 标准模型或使用 LlamaAttention 的自定义模型：直接调用（不使用 attn_mode）
            ...
            for head in HEADS_O_PROJ:
                proj_in = ret[head].input[0].squeeze().detach().cpu()
                seq_len = proj_in.shape[0]
                proj_in = proj_in.view(seq_len, num_heads, head_dim)
                head_wise_hidden_states.append(proj_in)
            head_wise_hidden_states = torch.stack(head_wise_hidden_states, dim = 0).squeeze().float().numpy()
...
def get_llama_activations_pyvene(collected_model, collectors, prompt, device):
    ...
    head_wise_hidden_states = torch.stack([torch.tensor(h) for h in head_wise_hidden_states], dim=0).squeeze().float().numpy()
    return hidden_states, head_wise_hidden_states, mlp_wise_hidden_states
...
def alt_tqa_evaluate(models, metric_names, input_path, output_path, summary_path, device='cpu', verbose=False, preset='qa', interventions={}, intervention_fn=None, cache_dir=None, separate_kl_device=None, orig_model=None, instruction_prompt="default", many_shot_prefix=None, judge_name=None, info_name=None): 
    ...
    # OpenAI is only needed for GPT-3 models
    if openai is None:
        print("Warning: openai package not installed. GPT-3 model evaluation will be skipped.")
    else:
    ...
    for mdl in models.keys(): 
        ...
        if mdl in ['ada', 'babbage', 'curie', 'davinci']:  # gpt-3 models
            if openai is None:
                print(f"Warning: Skipping {mdl} evaluation - openai package not installed.")
                continue
            try:
